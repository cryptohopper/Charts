...
function computeAO(candles: CandleData[]): number[] {
  // Median price = (high + low) / 2
  const median = candles.map((c) => (c.high + c.low) / 2);
  const sma5 = computeSMA(median, 5);
  const sma34 = computeSMA(median, 34);
...
export interface AOSettings {
  color1: string; // declining bars (red)
  color2: string; // rising bars (teal/green)
  lineWidth: number;
...
const DEFAULT_AO_SETTINGS: AOSettings = {
  color1: "#DE5537",
  color2: "#00B1C7",
  lineWidth: 1.5,
  plotStyle: "columns",
...
const PLOT_STYLES: { value: AOSettings["plotStyle"]; label: string; icon: React.ReactNode }[] = [
...
interface AOSettingsModalProps {
  open: boolean;
  onClose: () => void;
  settings: AOSettings;
  onApply: (s: AOSettings) => void;
}

function AOSettingsModal({ open, onClose, settings, onApply }: AOSettingsModalProps) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      >
        ...
        <div className="h-[40px] flex items-center justify-between px-[15px] rounded-t-md" style={{ backgroundColor: 'var(--hc-modal-header)' }}>
          ...
          <button onClick={onClose} className="w-[24px] h-[24px] flex items-center justify-center rounded">
        ...
        <div className="p-[15px]">
          {tab === "inputs" ? (
            ...
            <div className="flex flex-col gap-[16px]">
              {/* Color 1 (declining) */}
              <div className="flex items-center justify-between">
                <span className="text-[13px] text-[#1f1f1f] font-['Source_Sans_3',sans-serif]">Color 1</span>
                <div className="flex items-center gap-[8px]">
                  <label className="relative w-[28px] h-[28px] rounded border border-[#efeff4] cursor-pointer overflow-hidden">
                    <div className="absolute inset-[3px] rounded-sm" style={{ backgroundColor: local.color1 }} />
                    <input
                      type="color"
                      value={local.color1}
                      onChange={(e) => setLocal((s) => ({ ...s, color1: e.target.value }))}
                  ...
                  <div className="relative" ref={plotRef}>
                    ...
                    >
                      ...
                      <span className="text-[12px] font-['Source_Sans_3',sans-serif]">{selectedPlotStyle?.label || "Columns"}</span>
                      <svg width="8" height="5" viewBox="0 0 8 5" fill="none">
                        <path d="M0.5 0.5L4 4L7.5 0.5" stroke="#8E8E93" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </button>
              ...
              {/* Color 2 (rising) */}
              <div className="flex items-center justify-between">
                <span className="text-[13px] text-[#1f1f1f] font-['Source_Sans_3',sans-serif]">Color 2</span>
                <div className="flex items-center gap-[8px]">
                  <label className="relative w-[28px] h-[28px] rounded border border-[#efeff4] cursor-pointer overflow-hidden">
                    <div className="absolute inset-[3px] rounded-sm" style={{ backgroundColor: local.color2 }} />
                    <input
                      type="color"
                      value={local.color2}
                      onChange={(e) => setLocal((s) => ({ ...s, color2: e.target.value }))}
                      className="absolute inset-0 opacity-0 cursor-pointer"
                    />
                  </label>
                </div>
        ...
        <div className="flex items-center justify-between px-[15px] pb-[15px]">
          <button
            onClick={() => setLocal(DEFAULT_AO_SETTINGS)}
            className="text-[12px] text-[#8e8e93] font-['Source_Sans_3',sans-serif] hover:text-[#1f1f1f]"
          >
            Reset Settings
          </button>
          <div className="flex gap-[8px]">
            <button onClick={onClose} className="h-[30px] px-[16px] bg-white border border-[#babac1] rounded-sm text-[13px] text-[#8e8e93] font-['Source_Sans_3',sans-serif] hover:bg-gray-50">
              Cancel
            </button>
            <button
...
interface AOIndicatorPaneProps {
  data: CandleData[];
  scrollOffset: number;
...
export function AOIndicatorPane({
  data,
  scrollOffset,
  visibleCandles,
  chartWidth,
  ...
  isFullScreen,
  onToggleFullScreen,
}: AOIndicatorPaneProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const { isDark } = useTheme();
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 });
  const [hovered, setHovered] = useState(false);
  const [settings, setSettings] = useState<AOSettings>(DEFAULT_AO_SETTINGS);
  ...
  const aoValues = useMemo(() => computeAO(data), [data]);
  ...
  const visibleAO = useMemo(
    () => aoValues.slice(startIdx, startIdx + visibleCandles + 1),
    [aoValues, startIdx, visibleCandles]
  );

  const currentValue = visibleAO.length > 0 ? visibleAO[visibleAO.length - 1] : 0;
  ...
  // Determine bar color: rising = color2, declining = color1
  const getBarColor = useCallback((values: number[], idx: number): string => {
    if (idx === 0) return values[idx] >= 0 ? settings.color2 : settings.color1;
    return values[idx] >= values[idx - 1] ? settings.color2 : settings.color1;
  ...
  const drawChart = useCallback(() => {
    ...
    if (visibleAO.length < 2) return;

    // Find min/max
    let minVal = Infinity, maxVal = -Infinity;
    for (const v of visibleAO) {
    ...
    if (pStyle === "columns" || pStyle === "histogram") {
      // Histogram / columns bars
      const barW = Math.max(1, candleStep * 0.65);
      for (let i = 0; i < visibleAO.length; i++) {
        const x = i * candleStep + candleStep / 2 - barW / 2;
        const y = valToY(visibleAO[i]);
        const color = getBarColor(visibleAO, i);
        ...
        if (pStyle === "columns") {
          ...
          ctx.strokeRect(x, Math.min(y, zeroY), barW, Math.abs(y - zeroY));
        } else {
          ctx.fillStyle = color;
          ctx.fillRect(x, Math.min(y, zeroY), barW, Math.abs(y - zeroY));
        }
      }
    } else if (pStyle === "line" || pStyle === "line_with_breaks") {
      ...
      gradient.addColorStop(0, settings.color2 + "25");
      gradient.addColorStop(1, settings.color2 + "05");
      ctx.fillStyle = gradient;
      ctx.beginPath();
      ctx.moveTo(candleStep / 2, valToY(visibleAO[0]));
      for (let i = 1; i < visibleAO.length; i++) {
        ctx.lineTo(i * candleStep + candleStep / 2, valToY(visibleAO[i]));
      }
      const lastX = (visibleAO.length - 1) * candleStep + candleStep / 2;
      ...
      for (let i = 0; i < visibleAO.length; i++) {
        ...
        const y = valToY(visibleAO[i]);
        if (i === 0) { ctx.moveTo(x, y); }
        else { ctx.lineTo(x, valToY(visibleAO[i - 1])); ctx.lineTo(x, y); }
      }
      ctx.stroke();
      ...
      for (let i = 0; i < visibleAO.length; i++) {
        const x = i * candleStep + candleStep / 2;
        const y = valToY(visibleAO[i]);
        ctx.fillStyle = getBarColor(visibleAO, i);
        ctx.beginPath();
      ...
      for (let i = 0; i < visibleAO.length; i++) {
        const x = i * candleStep + candleStep / 2;
        const y = valToY(visibleAO[i]);
        ctx.strokeStyle = getBarColor(visibleAO, i);
        ctx.beginPath();
        ctx.moveTo(x - s, y - s); ctx.lineTo(x + s, y + s);
        ctx.moveTo(x + s, y - s); ctx.lineTo(x - s, y + s);
        ctx.stroke();
      }
    } else if (pStyle === "area_with_breaks") {
      ctx.strokeStyle = settings.color2;
      ctx.lineWidth = settings.lineWidth;
      ctx.beginPath();
      for (let i = 0; i < visibleAO.length; i++) {
    ...
    if (mouseX !== null && mouseX < cw) {
      ...
      if (candleIdx >= 0 && candleIdx < visibleAO.length) {
        const dotY = valToY(visibleAO[candleIdx]);
        ctx.fillStyle = getBarColor(visibleAO, candleIdx);
        ctx.beginPath();
        ctx.arc(candleIdx * candleStep + candleStep / 2, dotY, 3, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  }, [dimensions, visibleAO, candleStep, settings, visible, mouseX, formatValue, getBarColor, isDark]);
  ...
  const hoveredValue = useMemo(() => {
    ...
    const candleIdx = Math.floor(mouseX / candleStep);
    if (candleIdx >= 0 && candleIdx < visibleAO.length) return visibleAO[candleIdx];
    return null;
  }, [mouseX, candleStep, visibleAO]);

  const displayValue = hoveredValue ?? currentValue;
  // Color based on rising/falling for display
  const displayColor = useMemo(() => {
    if (hoveredValue !== null) {
      const candleIdx = Math.floor((mouseX ?? 0) / candleStep);
      return getBarColor(visibleAO, candleIdx);
    }
    // Current (last bar)
    return getBarColor(visibleAO, visibleAO.length - 1);
  }, [hoveredValue, mouseX, candleStep, visibleAO, getBarColor]);
        ...
        <div className="h-[30px] flex items-center px-[10px] gap-[6px] shrink-0" style={{ borderBottom: '1px solid var(--hc-border)', backgroundColor: 'var(--hc-surface)' }}>
          ...
          <div className="ml-[4px] flex items-center gap-[6px]">
            <span
              ...
              style={{ color: displayColor, backgroundColor: displayColor + "15", fontWeight: 600 }}
            >
