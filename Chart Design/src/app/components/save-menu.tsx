...
/* --- Modal Shell (reused by Save/Rename/Share/Load modals) --- */
...
function SaveLayoutModal({ open, onClose, onSave, defaultName = "Unnamed Layout 1" }: SaveLayoutModalProps) {
...
function RenameModal({ open, onClose, onRename, currentName }: RenameModalProps) {
...
function CopyRow({ label, url }: { label: React.ReactNode; url: string }) {
    ...
    <div>
      ...
      <div className="flex gap-[8px]">
        <div className="flex-1 h-[40px] rounded flex items-center px-[12px] overflow-hidden" style={{ backgroundColor: 'var(--hc-input-bg)', border: '1px solid var(--hc-border)' }}>
          <span className="text-[14px] font-['Source_Sans_3',sans-serif] truncate" style={{ color: 'var(--hc-text-secondary)' }}>{url}</span>
        </div>
        <button onClick={handleCopy} className="h-[40px] w-[80px] shrink-0 bg-[#00b1c7] rounded-[3px] text-[14px] text-white font-['Source_Sans_3',sans-serif] hover:bg-[#009aad] transition-colors" style={{ fontWeight: 600 }}>
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
    </div>
...
function ShareModal({ open, onClose }: ShareModalProps) {
  const baseUrl = "http://www.cryptohopper.com/dsfdfsffsg//gc";
  const labelStyle = { color: 'var(--hc-text)' } as React.CSSProperties;
  return (
    <ModalShell open={open} onClose={onClose} width={406}>
      <ModalHeader title="Share" onClose={onClose} />
      <div className="px-[15px] pt-[12px] pb-[20px] flex flex-col gap-[16px]">
        <CopyRow label={<p className="text-[14px] font-['Source_Sans_3',sans-serif] leading-[20px]" style={labelStyle}>Paste this code as high in the <span style={{ fontWeight: 600 }}>&lt;head&gt;</span> of the page as possible:</p>} url={baseUrl} />
        <CopyRow label={<p className="text-[14px] font-['Source_Sans_3',sans-serif] leading-[20px]" style={labelStyle}>Additionally paste this code in <span style={{ fontWeight: 600 }}>&lt;body&gt;</span> tag:</p>} url={baseUrl} />
        <CopyRow label={<p className="text-[14px] font-['Source_Sans_3',sans-serif] leading-[20px]" style={labelStyle}>Or you can paste widget as iframe:</p>} url={baseUrl} />
      </div>
    </ModalShell>
  );
}
...
interface LoadLayoutModalProps { open: boolean; onClose: () => void; onLoad: (layout: SavedLayout) => void; }

function LoadLayoutModal({ open, onClose, onLoad }: LoadLayoutModalProps) {
            ...
            >
              ...
              <div className="h-[40px] flex items-center justify-between px-[15px] rounded-t-md shrink-0" style={{ backgroundColor: 'var(--hc-modal-header)' }}>
                ...
                <div className="flex items-center gap-[4px]">
                  ...
                  <button onClick={onClose} className="w-[24px] h-[24px] flex items-center justify-center rounded transition-colors">
                    <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M10.5 0.5L0.5 10.5M0.5 0.5L10.5 10.5" stroke={IC} strokeLinecap="round" strokeLinejoin="round" /></svg>
                  </button>
                </div>
              </div>
                    ...
                    <div className="px-[15px] py-[8px]" style={{ borderBottom: '1px solid var(--hc-border)' }}>
                      <div className="relative">
                        <input
                          ...
                          className="w-full h-[32px] px-[10px] pr-[28px] rounded text-[13px] font-['Source_Sans_3',sans-serif] outline-none transition-colors"
                          style={{ backgroundColor: 'var(--hc-input-bg)', border: '1px solid var(--hc-border)', color: 'var(--hc-text)' }}
                          onFocus={(e) => e.currentTarget.style.borderColor = '#00b1c7'}
                          onBlur={(e) => e.currentTarget.style.borderColor = 'var(--hc-border)'}
                        />
                        {searchQuery && (
                          <button onClick={() => { setSearchQuery(""); searchInputRef.current?.focus(); }} className="absolute right-[6px] top-1/2 -translate-y-1/2 w-[16px] h-[16px] flex items-center justify-center">
                            <svg width="7" height="7" viewBox="0 0 11 11" fill="none"><path d="M10.5 0.5L0.5 10.5M0.5 0.5L10.5 10.5" stroke="var(--hc-text-secondary)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" /></svg>
                          </button>
                        )}
                      </div>
                    </div>
