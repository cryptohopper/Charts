...
import { useChartStore, useUserStore, isPremiumIndicator, hasMinTier } from '@hopcharts/core';
import type { IndicatorConfig, IndicatorPlacement, SubscriptionTier } from '@hopcharts/core';
...
const INDICATORS: IndicatorDef[] = [
  { id: 'sma', name: 'Simple Moving Average', shortName: 'SMA', description: 'Average price over a set number of periods', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'ema', name: 'Exponential Moving Average', shortName: 'EMA', description: 'Weighted moving average giving more weight to recent prices', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'wma', name: 'Weighted Moving Average', shortName: 'WMA', description: 'Linearly increasing weights to more recent data', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'dema', name: 'Double Exponential Moving Average', shortName: 'DEMA', description: 'Reduces lag compared to a single EMA', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'tema', name: 'Triple Exponential Moving Average', shortName: 'TEMA', description: 'Removes lag inherent in standard moving averages', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'vwma', name: 'Volume Weighted Moving Average', shortName: 'VWMA', description: 'Weighted average price based on volume', category: 'moving_averages', defaultParams: { period: 20 }, placement: 'overlay' },
  { id: 'rsi', name: 'Relative Strength Index', shortName: 'RSI', description: 'Measures the speed and magnitude of price changes', category: 'oscillators', defaultParams: { period: 14 }, placement: 'sub-pane' },
  { id: 'macd', name: 'Moving Average Convergence Divergence', shortName: 'MACD', description: 'Shows the relationship between two moving averages', category: 'oscillators', defaultParams: { fast: 12, slow: 26, signal: 9 }, placement: 'sub-pane' },
  { id: 'stoch', name: 'Stochastic Oscillator', shortName: 'Stoch', description: 'Compares closing price to the range over a given period', category: 'oscillators', defaultParams: { period: 14, kSmooth: 3, dSmooth: 3 }, placement: 'sub-pane' },
  { id: 'cci', name: 'Commodity Channel Index', shortName: 'CCI', description: 'Measures current price relative to the average price', category: 'oscillators', defaultParams: { period: 20 }, placement: 'sub-pane' },
  { id: 'bb', name: 'Bollinger Bands', shortName: 'BB', description: 'Upper and lower bands at standard deviations from a moving average', category: 'volatility', defaultParams: { period: 20, stdDev: 2 }, placement: 'overlay' },
  { id: 'atr', name: 'Average True Range', shortName: 'ATR', description: 'Measures market volatility over a given period', category: 'volatility', defaultParams: { period: 14 }, placement: 'sub-pane' },
  { id: 'kc', name: 'Keltner Channels', shortName: 'KC', description: 'Volatility-based bands set above and below an EMA', category: 'volatility', defaultParams: { period: 20, multiplier: 2 }, placement: 'overlay' },
  { id: 'obv', name: 'On-Balance Volume', shortName: 'OBV', description: 'Uses volume flow to predict price changes', category: 'volume', defaultParams: {}, placement: 'sub-pane' },
  { id: 'vwap', name: 'Volume Weighted Average Price', shortName: 'VWAP', description: 'Average price weighted by volume for the day', category: 'volume', defaultParams: {}, placement: 'overlay' },
  { id: 'adx', name: 'Average Directional Index', shortName: 'ADX', description: 'Measures the strength of a trend regardless of direction', category: 'trend', defaultParams: { period: 14 }, placement: 'sub-pane' },
  { id: 'ichimoku', name: 'Ichimoku Cloud', shortName: 'Ichimoku', description: 'Shows support/resistance, momentum, and trend direction', category: 'trend', defaultParams: { tenkan: 9, kijun: 26, senkou: 52 }, placement: 'overlay' },
  { id: 'psar', name: 'Parabolic SAR', shortName: 'PSAR', description: 'Determines trend direction and reversal points', category: 'trend', defaultParams: { step: 0.02, max: 0.2 }, placement: 'overlay' },
  { id: 'supertrend', name: 'Supertrend', shortName: 'ST', description: 'Trend-following indicator based on ATR', category: 'trend', defaultParams: { period: 10, multiplier: 3 }, placement: 'overlay' },
];
...
export function IndicatorMenu() {
  ...
  const toggleIndicator = (def: IndicatorDef) => {
    if (isPremiumIndicator(def.id) && !isHero) return;
    if (activeIndicatorIds.has(def.id)) {
      removeIndicator(def.id);
    } else {
  ...
  return (
    <div className="relative h-full" ref={panelRef}>
      ...
      >
        ...
        {indicators.length > 0 && (
          <span
            className="ml-[2px] w-[18px] h-[18px] rounded-full text-white text-[11px] font-['Source_Sans_3',sans-serif] flex items-center justify-center"
      ...
      {isOpen && (
        ...
        >
          {/* LEFT SIDEBAR */}
          <div
            className="w-[180px] flex flex-col shrink-0"
            style={{
              backgroundColor: 'var(--hc-modal-header)',
              ...
              {SIDEBAR_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[13px] font-['Source_Sans_3',sans-serif]">
                      {item.label}
                    </span>
                    {item.id === 'favorites' && favorites.size > 0 && (
                      <span
                        className="ml-auto text-[11px] font-['Source_Sans_3',sans-serif]"
                        style={{
                          color: isActive
                            ? 'var(--hc-accent)'
                            : 'var(--hc-text-muted)',
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center justify-between px-[16px] pb-[8px] shrink-0">
              <span
                className="text-[12px] font-['Source_Sans_3',sans-serif] uppercase tracking-wider"
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              {filtered.length === 0 ? (
                ...
                filtered.map((indicator) => {
                  const isActive = activeIndicatorIds.has(indicator.id);
                  const isFav = favorites.has(indicator.id);
                  const premium = isPremiumIndicator(indicator.id);
                  const locked = premium && !isHero;
                  return (
                    <div
                      key={indicator.id}
                    ...
                    >
                      ...
                      >
                        <span
                          className="text-[11px] font-['Source_Sans_3',sans-serif] font-semibold"
                          style={{
                            color: isActive
                              ? 'var(--hc-accent)'
                              : 'var(--hc-text-secondary)',
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-[6px]">
                          ...
                          {premium && (
                            <span
                              ...
                              style={{
                                backgroundColor: locked ? 'var(--hc-surface-active)' : 'rgba(0,177,199,0.1)',
                                color: locked ? 'var(--hc-text-muted)' : 'var(--hc-accent)',
                              }}
                            >
                              Hero
                            </span>
                        ...
                        <p
                          className="text-[12px] font-['Source_Sans_3',sans-serif] truncate mt-[1px]"
                      ...
                      {/* Add/Remove or Lock */}
                      {locked ? (
                        <div className="shrink-0 px-[12px] h-[30px] rounded-md flex items-center gap-[4px] text-[13px] font-['Source_Sans_3',sans-serif]"
                          style={{ backgroundColor: 'var(--hc-surface-active)', color: 'var(--hc-text-muted)' }}
                          title="Requires Hero subscription"
                        >
